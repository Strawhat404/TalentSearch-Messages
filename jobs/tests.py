...
class JobViewTests(APITestCase):
    ...
    def test_create_job_no_profile(self):
        ...
        response = self.client.post('/api/jobs/', data, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.data['error'], 'You must create a profile before posting a job.')
...
class ApplicationViewTests(APITestCase):
    def setUp(self):
        ...
        self.user = User.objects.create_user(email='test@example.com', password='testpass123')
        self.profile = Profile.objects.create(user=self.user, name='Test Profile')
        self.applicant = User.objects.create_user(email='applicant@exam.com', password='testpass123')
        self.applicant_profile = Profile.objects.create(user=self.applicant, name='Applicant Profile')
        self.token = Token.objects.create(user=self.user)
        self.client.credentials(HTTP_AUTHORIZATION=f'Token {self.token.key}')
        self.job = Job.objects.create(
            profile_id=self.profile,
    ...
    def test_apply_to_job(self):
        ...
        self.client.credentials(HTTP_AUTHORIZATION=f'Token {Token.objects.create(user=self.applicant).key}')
        ...
        application = Application.objects.get(job=self.job, profile_id=self.applicant_profile)
        self.assertEqual(application.opportunity_description, 'I am a great fit for this role.')
    ...
    def test_apply_to_job_no_profile(self):
        ...
        token = Token.objects.create(user=applicant_without_profile)
        self.client.credentials(HTTP_AUTHORIZATION=f'Token {token.key}')
        data = {'opportunity_description': 'I am a great fit.'}
        response = self.client.post(f'/api/jobs/{self.job.id}/apply/', data, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.data['error'], 'You must create a profile before applying for a job.')
    ...
    def test_apply_to_job_expired(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.data['error'], 'Job has expired')

    def test_apply_to_own_job(self):
        data = {'opportunity_description': 'I am a great fit.'}
        response = self.client.post(f'/api/jobs/{self.job.id}/apply/', data, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.data['error'], 'You cannot apply to your own job.')
    ...
    def test_apply_to_job_missing_description(self):
        ...
        response = self.client.post(f'/api/jobs/{self.job.id}/apply/', data, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.data['error'], 'Opportunity description is missing')
    ...
    def test_get_applicants(self):
        ...
        self.assertEqual(applicant_data['applicant_name'], 'Applicant Profile')
        self.assertEqual(applicant_data['applicant_email'], 'applicant@exam.com')
        self.assertEqual(applicant_data['opportunity_description'], 'Test application')

    def test_get_applicants_unauthorized(self):
        other_user = User.objects.create_user(email='other@exam.com', password='testpass123')
        other_profile = Profile.objects.create(user=other_user, name='Other Profile')
