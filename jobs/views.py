...
#         if request.user.is_superuser or request.user.is_staff:  # Check superuser or staff
...
class JobDetailView(APIView):
    ...
    def put(self, request, job_id):
        ...
        """
        try:
            job = Job.objects.get(id=job_id)  # Check existence without user filter
        except Job.DoesNotExist:
            return Response({"message": "Job not found."}, status=status.HTTP_404_NOT_FOUND)

        # Check if user is admin (superuser or staff) or the creator
        if not (job.user_id == request.user):
    ...
    def delete(self, request, job_id):
        ...
        # Check if user is admin (superuser or staff) or the creator
        if not (request.user.is_superuser or request.user.is_staff or job.user_id == request.user):
