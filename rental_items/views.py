...
from rest_framework.views import APIView
from django_filters.rest_framework import DjangoFilterBackend
from .models import RentalItem, RentalItemRating, Wishlist, FeaturedItemAuditLog
from .serializers import (
    RentalItemSerializer,
    RentalItemListSerializer,
    RentalItemUpdateSerializer,
...
class RentalItemViewSet(ModelViewSet):
    ...
    def set_featured(self, request, pk=None):
        """Set or unset the featured status of a rental item and log the change"""
        rental_item = self.get_object()
        featured = request.data.get('featured', None)
        if featured is None:
            return Response(
                {'error': 'Featured status must be provided (true or false)'},
        ...
        # Log the change
        FeaturedItemAuditLog.objects.create(
            rental_item=rental_item,
            previous_status=rental_item.featured_item,
            new_status=featured,
        ...
        # Update the featured_item status
        rental_item.featured_item = featured
        rental_item.save()

        return Response({
            'id': str(rental_item.id),
            'message': f'Rental item {"featured" if featured else "unfeatured"} successfully.',
...
class FeaturedItemAuditLogView(APIView):
    permission_classes = [IsAdminUser]

    @swagger_auto_schema(
        tags=['rental_items'],
        summary='Get featured item audit logs',
        description='Retrieve audit logs for featured item changes for a specific rental item or all logs.',
        parameters=[
            openapi.Parameter(
                'rental_item_id',
                openapi.IN_QUERY,
                description='UUID of the rental item to filter logs (optional)',
                ...
                schema=openapi.Schema(
                    ...
                    items=openapi.Schema(
                        ...
                        properties={
                            ...
                            'rental_item': openapi.Schema(type=openapi.TYPE_STRING, format=openapi.FORMAT_UUID),
                            'rental_item_name': openapi.Schema(type=openapi.TYPE_STRING),
                            'previous_status': openapi.Schema(type=openapi.TYPE_BOOLEAN),
                            'new_status': openapi.Schema(type=openapi.TYPE_BOOLEAN),
                            'changed_by': openapi.Schema(type=openapi.TYPE_STRING, format=openapi.FORMAT_UUID),
                            'changed_by_email': openapi.Schema(type=openapi.TYPE_STRING),
                            'changed_at': openapi.Schema(type=openapi.TYPE_STRING, format=openapi.FORMAT_DATETIME),
    ...
    def get(self, request):
        rental_item_id = request.query_params.get('rental_item_id')
        if rental_item_id:
            try:
                rental_item = get_object_or_404(RentalItem, id=rental_item_id)
                logs = FeaturedItemAuditLog.objects.filter(rental_item=rental_item)
            ...
            logs = FeaturedItemAuditLog.objects.all()

        serializer = FeaturedItemAuditLogSerializer(logs, many=True)
